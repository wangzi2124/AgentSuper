...
def main() -> int:
    ...
    print(f"压缩判断次数  : {g['pre_compact']}，其中触发压缩(>19,046): {g['comp_triggered']}")

    # 每请求画像
    rows = []
    for b, evs in batches.items():
        u = summarize(evs)
        rounds = len([e for e in evs if e["event"] == "graph.round_start"])
    ...
    if target is not None and target in batches:
        evs = batches[target]
        starts = {e["ts"]: e for e in evs if e["event"] == "graph.round_start"}
        readys = [e for e in evs if e["event"] == "graph.round_ready"]
