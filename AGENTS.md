...
- **User identity signature** (optional, `app/auth.py` + `app/api/auth.py`): set `AUTH_TOKEN_SECRET` in `.env` to enable. Trust-on-first-use: frontend generates random `user_id`+`device_secret` → `POST /api/auth/register` (server stores only HMAC hash) → `POST /api/auth/token` for a signed token (`base64(uid).base64(exp).hmac(uid:exp:reg_hash)`). `AuthMiddleware` then requires `X-User-Id` + `X-Auth-Token` on every `/api/*` request (skips `/api/auth/*`, non-`/api`, OPTIONS). When `AUTH_TOKEN_SECRET` is unset all checks are off (default local behavior). Frontend auto-handles via `frontend/src/api/auth.ts` (`ensureAuth()` called in `main.ts`, headers injected in `fetch.ts:addAuthHeaders` and `documents.ts` upload). Config: `auth_token_ttl` (30d), `auth_users_path` (`data/auth_users.json`).
